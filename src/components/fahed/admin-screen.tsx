...
export default function AdminScreen() {
  ...
  const [viewKycPhoto, setViewKycPhoto] = useState<{ type: 'id' | 'selfie' | 'idFront' | 'idBack'; url: string } | null>(null);
  ...
  // Bulk Gift Codes
  const [showBulkGiftCodes, setShowBulkGiftCodes] = useState(false);
  const [bulkGiftProviderId, setBulkGiftProviderId] = useState('');
  const [bulkGiftPackageName, setBulkGiftPackageName] = useState('');
  const [bulkGiftAmount, setBulkGiftAmount] = useState(0);
  const [bulkGiftCurrency, setBulkGiftCurrency] = useState<'YER' | 'SAR' | 'USD'>('YER');
  const [bulkGiftMaxUses, setBulkGiftMaxUses] = useState(1);
  const [bulkGiftExpiresAt, setBulkGiftExpiresAt] = useState('');
  const [bulkCodesText, setBulkCodesText] = useState('');
  const [bulkCodesFilter, setBulkCodesFilter] = useState<'all' | 'active' | 'inactive' | 'expired' | 'used'>('all');
  const [bulkCodesProviderFilter, setBulkCodesProviderFilter] = useState<string>('all');
  const [giftCodeTab, setGiftCodeTab] = useState<'single' | 'bulk' | 'manage'>('single');
    ...
    const unsubscribe = onValue(usersRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const usersList = Object.entries(data).map(([key, val]: [string, any]) => ({
          ...
          idPhotoUrl: val.idPhotoUrl, selfieUrl: val.selfieUrl, idCardFront: val.idCardFront, idCardBack: val.idCardBack, avatar: val.avatar,
  ...
  // Bulk Gift Code handler
  const handleAddBulkGiftCodes = async () => {
    if (!bulkCodesText.trim() || bulkGiftAmount <= 0) return;
    const codes = bulkCodesText.trim().split('\n').map(c => c.trim()).filter(c => c.length > 0);
    if (codes.length === 0) return;

    const provider = providers.find(p => p.id === bulkGiftProviderId);
    const expiresAt = bulkGiftExpiresAt || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    ...
    try {
      for (const code of codes) {
        ...
        const giftCode: GiftCodeData = {
          id,
          code,
          amount: bulkGiftAmount,
          currency: bulkGiftCurrency,
          maxUses: bulkGiftMaxUses,
          usedCount: 0,
          expiresAt,
          isActive: true,
          createdAt: new Date().toISOString(),
          createdById: user?.id || 'admin',
          providerId: bulkGiftProviderId || undefined,
          providerName: provider?.name || '',
          packageName: bulkGiftPackageName || undefined,
        };
        await set(ref(database, `giftCodes/${id}`), giftCode);
      }
      setBulkCodesText('');
      setBulkGiftProviderId('');
      setBulkGiftPackageName('');
      setBulkGiftAmount(0);
      setBulkGiftCurrency('YER');
      setBulkGiftMaxUses(1);
      setBulkGiftExpiresAt('');
      setShowBulkGiftCodes(false);
      addAuditEntry(`تم إضافة ${codes.length} كود هدية دفعة واحدة`);
    } catch {}
  };
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: isDark ? '#0F0F0F' : '#F5F5F5', direction: 'rtl' }}>
              ...
              <motion.div key="kyc" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
                ...
                {kycUsers.map((u) => (
                  <div key={u.id} className="rounded-2xl p-4" style={{ ...cardStyle, borderRight: '3px solid #3B82F6' }}>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      {u.idCardFront ? (
                        <button onClick={() => setViewKycPhoto({ type: 'idFront', url: u.idCardFront! })} className="rounded-xl overflow-hidden border-2 transition-all hover:opacity-80" style={{ borderColor: 'rgba(59,130,246,0.3)' }}>
                          <div className="relative">
                            {u.idCardFront.startsWith('data:') ? (
                              <img src={u.idCardFront} alt="صورة أمامية" className="w-full h-24 object-cover" />
                            ) : (
                              <img src={u.idCardFront} alt="صورة أمامية" className="w-full h-24 object-cover" crossOrigin="anonymous" />
                            )}
                            <div className="absolute bottom-0 left-0 right-0 py-1 px-2" style={{ background: 'rgba(0,0,0,0.6)' }}>
                              <span className="text-[9px] text-white font-medium">صورة الأمامية</span>
                            </div>
                          </div>
                        </button>
                      ) : u.idPhotoUrl ? (
                        <button onClick={() => setViewKycPhoto({ type: 'id', url: u.idPhotoUrl! })} className="rounded-xl overflow-hidden border-2 transition-all hover:opacity-80" style={{ borderColor: 'rgba(59,130,246,0.3)' }}>
                          <div className="relative">
                            {u.idPhotoUrl.startsWith('data:') ? (
                              <img src={u.idPhotoUrl} alt="صورة الوثيقة" className="w-full h-24 object-cover" />
                            ) : (
                              <img src={u.idPhotoUrl} alt="صورة الوثيقة" className="w-full h-24 object-cover" crossOrigin="anonymous" />
                            )}
                            <div className="absolute bottom-0 left-0 right-0 py-1 px-2" style={{ background: 'rgba(0,0,0,0.6)' }}>
                              <span className="text-[9px] text-white font-medium">صورة الوثيقة</span>
                            </div>
                          </div>
                        </button>
                      ) : (
                        <div className="rounded-xl h-24 flex flex-col items-center justify-center" style={{ background: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)', border: `2px dashed ${isDark ? '#333' : '#DDD'}` }}>
                          <ImageIcon size={16} color={isDark ? '#444' : '#CCC'} />
                          <span className="text-[8px] mt-1" style={{ color: isDark ? '#555' : '#AAA' }}>لا توجد صورة أمامية</span>
                        </div>
                      )}
                      {u.idCardBack ? (
                        <button onClick={() => setViewKycPhoto({ type: 'idBack', url: u.idCardBack! })} className="rounded-xl overflow-hidden border-2 transition-all hover:opacity-80" style={{ borderColor: 'rgba(139,92,246,0.3)' }}>
                          <div className="relative">
                            {u.idCardBack.startsWith('data:') ? (
                              <img src={u.idCardBack} alt="صورة خلفية" className="w-full h-24 object-cover" />
                            ) : (
                              <img src={u.idCardBack} alt="صورة خلفية" className="w-full h-24 object-cover" crossOrigin="anonymous" />
                            )}
                            <div className="absolute bottom-0 left-0 right-0 py-1 px-2" style={{ background: 'rgba(0,0,0,0.6)' }}>
                              <span className="text-[9px] text-white font-medium">صورة الخلفية</span>
                            </div>
                          </div>
                        </button>
                      ) : u.selfieUrl ? (
                        <button onClick={() => setViewKycPhoto({ type: 'selfie', url: u.selfieUrl! })} className="rounded-xl overflow-hidden border-2 transition-all hover:opacity-80" style={{ borderColor: 'rgba(139,92,246,0.3)' }}>
                          <div className="relative">
                            {u.selfieUrl.startsWith('data:') ? (
                              <img src={u.selfieUrl} alt="الصورة الشخصية" className="w-full h-24 object-cover" />
                            ) : (
                              <img src={u.selfieUrl} alt="الصورة الشخصية" className="w-full h-24 object-cover" crossOrigin="anonymous" />
                            )}
                            <div className="absolute bottom-0 left-0 right-0 py-1 px-2" style={{ background: 'rgba(0,0,0,0.6)' }}>
                              <span className="text-[9px] text-white font-medium">الصورة الشخصية</span>
                            </div>
                          </div>
                        </button>
                      ) : (
                        <div className="rounded-xl h-24 flex flex-col items-center justify-center" style={{ background: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)', border: `2px dashed ${isDark ? '#333' : '#DDD'}` }}>
                          <ImageIcon size={16} color={isDark ? '#444' : '#CCC'} />
                          <span className="text-[8px] mt-1" style={{ color: isDark ? '#555' : '#AAA' }}>لا توجد صورة خلفية</span>
                        </div>
                      )}
                    </div>
                    <div className="flex gap-2">
            ...
            {activeTab === 'giftCodes' && (
              <motion.div key="giftCodes" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
                ...
                <div className="flex gap-2 p-1 rounded-2xl" style={{ background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)' }}>
                  <button onClick={() => setGiftCodeTab('single')} className="flex-1 py-2 rounded-xl text-xs font-medium transition-all" style={{ background: giftCodeTab === 'single' ? '#E60000' : 'transparent', color: giftCodeTab === 'single' ? '#FFF' : isDark ? '#AAA' : '#888' }}>كود واحد</button>
                  <button onClick={() => setGiftCodeTab('bulk')} className="flex-1 py-2 rounded-xl text-xs font-medium transition-all" style={{ background: giftCodeTab === 'bulk' ? '#E60000' : 'transparent', color: giftCodeTab === 'bulk' ? '#FFF' : isDark ? '#AAA' : '#888' }}>إضافة أكواد</button>
                  <button onClick={() => setGiftCodeTab('manage')} className="flex-1 py-2 rounded-xl text-xs font-medium transition-all" style={{ background: giftCodeTab === 'manage' ? '#E60000' : 'transparent', color: giftCodeTab === 'manage' ? '#FFF' : isDark ? '#AAA' : '#888' }}>إدارة الأكواد</button>
                </div>
                ...
                {giftCodeTab === 'bulk' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
                    <div className="rounded-2xl p-4 space-y-3" style={cardStyle}>
                      <div className="flex items-center gap-2 mb-1">
                        <Package size={16} color="#E60000" />
                        <h3 className="text-sm font-bold" style={{ color: isDark ? '#FFF' : '#1a1a1a' }}>إضافة أكواد دفعة واحدة</h3>
                      </div>

                      {/* Provider selector */}
                      <div>
                        <label className="text-[10px] font-medium block mb-1" style={{ color: isDark ? '#AAA' : '#888' }}>المزود / الباقة</label>
                        <select value={bulkGiftProviderId} onChange={(e) => { setBulkGiftProviderId(e.target.value); const p = providers.find(pr => pr.id === e.target.value); if (p) setBulkGiftPackageName(p.name); }} className="w-full px-3 py-2.5 rounded-xl text-sm outline-none" style={inputStyle}>
                          <option value="">اختر المزود</option>
                          {providers.filter(p => p.categoryId === 'entertainment' || p.categoryId === 'cards').map(p => (
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1">
                          <label className="text-[10px] font-medium block mb-1" style={{ color: isDark ? '#AAA' : '#888' }}>المبلغ لكل كود</label>
                          <input type="number" placeholder="المبلغ" value={bulkGiftAmount || ''} onChange={(e) => setBulkGiftAmount(parseFloat(e.target.value) || 0)} className="w-full px-3 py-2.5 rounded-xl text-sm outline-none" style={inputStyle} dir="ltr" />
                        </div>
                        <div className="w-24">
                          <label className="text-[10px] font-medium block mb-1" style={{ color: isDark ? '#AAA' : '#888' }}>العملة</label>
                          <select value={bulkGiftCurrency} onChange={(e) => setBulkGiftCurrency(e.target.value as 'YER' | 'SAR' | 'USD')} className="w-full px-3 py-2.5 rounded-xl text-sm outline-none" style={inputStyle}>
                            <option value="YER">YER</option><option value="SAR">SAR</option><option value="USD">USD</option>
                          </select>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1">
                          <label className="text-[10px] font-medium block mb-1" style={{ color: isDark ? '#AAA' : '#888' }}>الحد الأقصى للاستخدام</label>
                          <input type="number" placeholder="1" value={bulkGiftMaxUses || ''} onChange={(e) => setBulkGiftMaxUses(parseInt(e.target.value) || 1)} className="w-full px-3 py-2.5 rounded-xl text-sm outline-none" style={inputStyle} dir="ltr" />
                        </div>
                        <div className="flex-1">
                          <label className="text-[10px] font-medium block mb-1" style={{ color: isDark ? '#AAA' : '#888' }}>تاريخ الانتهاء</label>
                          <input type="date" value={bulkGiftExpiresAt} onChange={(e) => setBulkGiftExpiresAt(e.target.value)} className="w-full px-3 py-2.5 rounded-xl text-sm outline-none" style={inputStyle} />
                        </div>
                      </div>

                      {/* Codes Textarea */}
                      <div>
                        <label className="text-[10px] font-medium block mb-1" style={{ color: isDark ? '#AAA' : '#888' }}>الأكواد (كود واحد في كل سطر)</label>
                        <textarea
                          placeholder={`HJ4IDBD\nJSBFEIDB\n4JDKWBD`}
                          value={bulkCodesText}
                        ...
                        {bulkCodesText.trim() && (
                          <p className="text-[10px] mt-1" style={{ color: '#8B5CF6' }}>
                            عدد الأكواد: {bulkCodesText.trim().split('\n').filter(c => c.trim()).length}
                          </p>
                        )}
                      </div>

                      <motion.button whileTap={{ scale: 0.95 }} onClick={handleAddBulkGiftCodes} disabled={!bulkCodesText.trim() || bulkGiftAmount <= 0}
                        className="w-full py-3 rounded-xl text-sm font-bold text-white flex items-center justify-center gap-2"
                        style={{ background: (!bulkCodesText.trim() || bulkGiftAmount <= 0) ? '#555' : '#E60000', opacity: (!bulkCodesText.trim() || bulkGiftAmount <= 0) ? 0.5 : 1 }}>
                        <Plus size={16} /><span>إضافة الأكواد</span>
                      </motion.button>
                    </div>
                  </motion.div>
                )}
                ...
                {giftCodeTab === 'manage' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
                    {/* Filters */}
                    <div className="flex gap-2">
                      <select value={bulkCodesProviderFilter} onChange={(e) => setBulkCodesProviderFilter(e.target.value)} className="flex-1 px-3 py-2 rounded-xl text-xs outline-none" style={inputStyle}>
                        <option value="all">جميع المزودين</option>
                        {[...new Set(giftCodes.filter(gc => gc.providerName).map(gc => gc.providerName!))].map(name => (
                          <option key={name} value={name}>{name}</option>
                        ))}
                        <option value="none">بدون مزود</option>
                      </select>
                      <select value={bulkCodesFilter} onChange={(e) => setBulkCodesFilter(e.target.value as any)} className="flex-1 px-3 py-2 rounded-xl text-xs outline-none" style={inputStyle}>
                    ...
                    {giftCodes
                      .filter(gc => {
                        if (bulkCodesProviderFilter !== 'all') {
                          if (bulkCodesProviderFilter === 'none' && gc.providerName) return false;
                          if (bulkCodesProviderFilter !== 'none' && gc.providerName !== bulkCodesProviderFilter) return false;
                        }
                        if (bulkCodesFilter === 'active') return gc.isActive && !(gc.expiresAt && new Date(gc.expiresAt) < new Date());
                        if (bulkCodesFilter === 'inactive') return !gc.isActive;
                        if (bulkCodesFilter === 'expired') return gc.expiresAt && new Date(gc.expiresAt) < new Date();
                        if (bulkCodesFilter === 'used') return gc.usedCount >= gc.maxUses;
                        return true;
                      })
                      .map((gc) => {
                        ...
                        return (
                          <div key={gc.id} className="rounded-2xl overflow-hidden" style={{
                            ...
                            <div className="p-4">
                              <div className="flex items-start justify-between mb-2">
                                <div className="flex items-center gap-2">
                                  <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: gc.providerId ? 'rgba(230,0,0,0.12)' : 'rgba(139,92,246,0.12)' }}>
                                    {gc.providerId ? <Gamepad2 size={18} color="#E60000" /> : <Gift size={18} color="#8B5CF6" />}
                                  </div>
                                  <div>
                                    ...
                                    {gc.providerName && <p className="text-[10px] mt-0.5" style={{ color: isDark ? '#888' : '#AAA' }}>{gc.providerName}{gc.packageName ? ` - ${gc.packageName}` : ''}</p>}
                                  </div>
                                </div>
                                <div className="flex items-center gap-2">
      ...
      <AnimatePresence>
        {viewKycPhoto && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.9)' }} onClick={() => setViewKycPhoto(null)}>
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
              <div className="rounded-2xl overflow-hidden" style={{ background: isDark ? '#1A1A1A' : '#FFF' }}>
                <div className="flex items-center justify-between p-4" style={{ borderBottom: isDark ? '1px solid #333' : '1px solid #EEE' }}>
                  <span className="text-sm font-bold" style={{ color: isDark ? '#FFF' : '#1a1a1a' }}>
                    {viewKycPhoto.type === 'idFront' ? 'صورة الأمامية' : viewKycPhoto.type === 'idBack' ? 'صورة الخلفية' : viewKycPhoto.type === 'id' ? 'صورة الوثيقة' : 'الصورة الشخصية'}
                  </span>
